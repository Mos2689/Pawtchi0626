...
// Letters go unsigned. There used to be a FOUNDER_DISPLAY_NAME constant here
// ("Pra and Mos") that the letter screens printed under "Regards,"; the two
// people behind Pawtchi still read and write every one, but their names no
// longer appear anywhere an owner sees. Every sentence that used to interpolate
// the name now says "we" instead — first person plural, no name required, no
// agreement trap.
...
export function renderCopy(campaign: CampaignKey, ctx: CopyContext): RenderedCopy {
  ...
  switch (campaign) {
    ...
    // sound like a different sender even though neither one is named: this
    // fires for a reply to a specific request, that one for a reply to a
    // letter. Keeping "letter" and "support request" out of each other's copy
    // is what keeps a routine support answer from reading like a personal one.
    case 'support_reply':
      return {
