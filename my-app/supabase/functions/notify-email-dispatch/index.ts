...
/**
 * Where the emailed CTA points.
 *
 * Not pawtchi.com. A tap on `https://pawtchi.com/app/home` opened a browser on
 * a blank page — the website serves no `/app/*` route and no released iOS build
 * claims the domain — so every CTA now goes to the public engagement-click
 * function, which answers 302 with a `pawtchi://` deep link. See
 * `_shared/email/links.ts` for the whole hop.
 *
 * Derived from SUPABASE_URL so it is correct in every environment without a
...
serve(async (req: Request) => {
  ...
  try {
    ...
    interface Plan {
      ...
      dedupeKey: string;
      /**
       * The `notification_history.id` this email will be claimed under, minted
       * BEFORE rendering rather than read back after.
       *
       * The CTA has to carry it — every click URL is
       * `engagement-click?s=<sendId>&t=<target>` — and the claim happens after
       * the whole batch is composed. Choosing the uuid here is what lets the
       * link name a row that does not exist yet; the upsert then inserts under
       * exactly this id, or loses the dedupe race and sends nothing at all.
       */
      sendId: string;
      to: string;
      subject: string;
    ...
    for (const c of candidates) {
      ...
      const templateInput = {
        ...
        baseUrl: PUBLIC_BASE,
        click: { functionsBase: FUNCTIONS_BASE, sendId },
      };
    ...
    const { data: claimed, error: claimError } = await admin
      ...
      .upsert(
        planned.map((p) => ({
          // The id the CTA already names. It has to be supplied rather than
          // generated by the default, or every click URL in the email would
          // point at a row that does not exist and attribution would be silent
          // and total — the exact failure mode the August rebuild exists to
          // prevent.
          id: p.sendId,
          user_id: p.userId,
          pet_id: p.petId,
