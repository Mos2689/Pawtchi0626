...
export default function GoalScreen() {
  ...
  const { species, breed, ageYears, weight } = petData;
  ...
  React.useEffect(() => {
    async function fetchIdealWeight() {
      ...
      if (data && data.ideal_weight_kg) {
        setTargetWeight(data.ideal_weight_kg);
  ...
  const handleComplete = async () => {
    ...
    const speciesVal = (petData.species === 'cat') ? 'cat' : 'dog'; // Enforce check constraint
    ...
    const { error } = await supabase.from('pets').insert({
      ...
      age_years: parseInt(petData.ageYears) || null,
    ...
    if (error) {
      ...
      // FIX: Await fetching the newly created pet to hydrate the store
      // BEFORE routing to the dashboard to prevent the double-loop redirect!
...
const styles = StyleSheet.create({
  ...
  sliderTrack: {
    position: 'absolute',
    top: 30, // vertically centered approximately
    width: '100%',
    height: 12,
    borderRadius: 6,
  },
  ...
  bottomFade: {
    ...
    width: '100%',
    height: 32,
    backgroundColor: 'rgba(255,255,255,0.8)', // Simulated gradient fade
  }
});
